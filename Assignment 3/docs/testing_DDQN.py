...
checkpoint_directory = f'./standard_model_eps_init{eps}_episode{episode_number}_DDQN.pth.tar'

run_as_ddqn = True #Decides DDQN(TRUE) or DQN(FALSE) will be used for calculating the loss
if run_as_ddqn:
    checkpoint_directory = f'./standard_model_eps_init{eps}_episode{episode_number}_DDQN.pth.tar'
else:
...
def compute_loss(state, action, reward, next_state, done):
    ...
    if run_as_ddqn == True:
        ...
        Q_online_max = torch.take(online_dqn(next_state), action_Q_online_max.long())
