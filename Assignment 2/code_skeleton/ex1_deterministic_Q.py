...
class DeterministicSARSA:
  ...
  def train_step(self, state, action, reward, next_state, next_action, done):
    ...
    if done:
        ...
        next_Q = np.amax(self.Q[next_state])
...
def tune_alpha_eps(gamma=0.9):
    ...
    plot_results(alpha_range, eps_range, 'alpha', 'epsilon', train_results, test_results, suptitle='Deterministic Q')
    plt.savefig(r'../plots/det_Q_alpha_eps.png')
...
def tune_alpha_gamma(eps=0.1):
  ...
  # DONE: Find and print the top-3 parameter combinations, that perform best during the test phase
    ...
    plot_results(alpha_range, gamma_range, 'alpha', 'gamma', train_results, test_results, suptitle='Deterministic Q')
    plt.savefig(r'../plots/det_Q_alpha_gamma.png')
