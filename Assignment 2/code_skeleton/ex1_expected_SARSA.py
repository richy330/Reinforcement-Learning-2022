...
class DeterministicSARSA:
  ...
  def action_probability(self, state):
      """return vector of pi(action | state) for all possible actions in state
      For epsilon greedy policies, this is a vector with epsilon probability 
      for each action, except the greedy action. The greedy action is taken
      with 100% probability - epsilon, plus the chance that the epsilon action
      chooses the greedy action"""
      Pi = np.ones(shape=self.num_actions)*self.eps/self.num_actions
      Pi[np.argmax(self.Q[state])] = 1 - self.eps * (1 + 1/self.num_actions)
      return Pi
  ...
  def train_step(self, state, action, reward, next_state, next_action, done):
    ...
    if done:
        ...
        next_Q = np.dot(self.Q[next_state], self.action_probability(next_state))
...
def tune_alpha_eps(gamma=0.9):
    ...
    plot_results(alpha_range, eps_range, 'alpha', 'epsilon', train_results, test_results, suptitle='Expected Sarsa')
    plt.savefig(r'../plots/expected_sarsa_alpha_eps.png')
...
def tune_alpha_gamma(eps=0.1):
  ...
  # DONE: Find and print the top-3 parameter combinations, that perform best during the test phase
    ...
    plot_results(alpha_range, gamma_range, 'alpha', 'gamma', train_results, test_results, suptitle='Expected Sarsa')
    plt.savefig(r'../plots/expected_sarsa_alpha_gamma.png')
